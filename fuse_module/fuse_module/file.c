			  ...
			  struct fuse_open_out *outargp)
{
	...
	if (fm->fc->handle_killpriv_v2 && (inarg.flags & O_TRUNC) &&
	    !capable(CAP_FSETID)) {
...
struct fuse_file *fuse_file_alloc(struct fuse_mount *fm)
{
	...
	ff->fm = fm;
	ff->release_args =
		kzalloc(sizeof(*ff->release_args), GFP_KERNEL_ACCOUNT);
	if (!ff->release_args) {
		kfree(ff);
		return NULL;
	}
...
int fuse_open_common(struct inode *inode, struct file *file, bool isdir)
{
	...
	struct fuse_conn *fc = fm->fc;
	int err;
	bool is_wb_truncate = (file->f_flags & O_TRUNC) && fc->atomic_o_trunc &&
			      fc->writeback_cache;
	bool dax_truncate = (file->f_flags & O_TRUNC) && fc->atomic_o_trunc &&
			    FUSE_IS_DAX(inode);
...
static struct fuse_writepage_args *
fuse_find_writeback(struct fuse_inode *fi, pgoff_t idx_from, pgoff_t idx_to)
{
	struct rb_node *n;
...
int fuse_fsync_common(struct file *file, loff_t start, loff_t end, int datasync,
		      int opcode)
{
					  ...
					  unsigned int npages)
{
	...
	if (ia) {
		ia->io = io;
		ia->ap.pages =
			fuse_pages_alloc(npages, GFP_KERNEL, &ia->ap.descs);
		if (!ia->ap.pages) {
			kfree(ia);
			ia = NULL;
		}
				     ...
				     unsigned int max_pages)
{
	...
	do {
		...
		size_t bytes =
			min_t(size_t, PAGE_SIZE - offset, iov_iter_count(ii));
				  ...
				  struct iov_iter *ii, loff_t pos)
{
	...
	do {
		...
		unsigned int nr_pages =
			fuse_wr_pages(pos, iov_iter_count(ii), fc->max_pages);
		...
		if (count <= 0) {
			...
			err = fuse_send_write_pages(&ia, iocb, inode, pos,
						    count);
			if (!err) {
...
static void fuse_send_writepage(struct fuse_mount *fm,
	...
	__releases(fi->lock) __acquires(fi->lock)
{
 ...
 * Called with fi->lock
 */
void fuse_flush_writepages(struct inode *inode) __releases(fi->lock)
	__acquires(fi->lock)
{
	struct fuse_mount *fm = get_fuse_mount(inode);
	struct fuse_inode *fi = get_fuse_inode(inode);
...
static struct fuse_writepage_args *
fuse_insert_writeback(struct rb_root *root, struct fuse_writepage_args *wpa)
{
			   ...
			   struct writeback_control *wbc)
{
	...
	err = -ENOMEM;
	data.orig_pages =
		kcalloc(fc->max_pages, sizeof(struct page *), GFP_NOFS);
	if (!data.orig_pages)
		goto out;
...
static loff_t fuse_lseek(struct file *file, loff_t offset, int whence)
{
	...
	struct fuse_file *ff = file->private_data;
	FUSE_ARGS(args);
	struct fuse_lseek_in inarg = { .fh = ff->fh,
				       .offset = offset,
				       .whence = whence };
...
static ssize_t fuse_direct_IO(struct kiocb *iocb, struct iov_iter *iter)
{
	...
	if (io->async && !io->write && offset + count > i_size) {
		iov_iter_truncate(iter,
				  fuse_round_up(ff->fm->fc, i_size - offset));
...
{
	int err = filemap_write_and_wait_range(inode->i_mapping, start,
					       LLONG_MAX);
				...
				loff_t length)
{
	...
	struct fuse_fallocate_in inarg = {
		.fh = ff->fh, .offset = offset, .length = length, .mode = mode
	};
	int err;
	bool lock_inode =
				      ...
				      size_t len, unsigned int flags)
{
	...
	struct fuse_conn *fc = fm->fc;
	FUSE_ARGS(args);
	struct fuse_copy_file_range_in inarg = { .fh_in = ff_in->fh,
						 .off_in = pos_in,
				    ...
				    size_t len, unsigned int flags)
{
	...
	ret = __fuse_copy_file_range(src_file, src_off, dst_file, dst_off, len,
				     flags);
