...
static struct fuse_req *fuse_get_req(struct fuse_mount *fm, bool for_background)
{
	...
	if (fuse_block_alloc(fc, for_background)) {
		err = -EINTR;
		if (wait_event_killable_exclusive(
			    fc->blocked_waitq,
...
static void queue_request_and_unlock(struct fuse_iqueue *fiq,
				     struct fuse_req *req) __releases(fiq->lock)
{
...
{
	if (page_mapcount(page) || page->mapping != NULL ||
	    (page->flags & PAGE_FLAGS_CHECK_AT_PREP &
	     ~(1 << PG_locked | 1 << PG_referenced | 1 << PG_uptodate |
	       1 << PG_lru | 1 << PG_active | 1 << PG_workingset |
	       1 << PG_reclaim | 1 << PG_waiters))) {
			  ...
			  unsigned offset, unsigned count, int zeroing)
{
	...
	while (count) {
		if (cs->write && cs->pipebufs && page) {
			...
			if (cs->move_pages && page && offset == 0 &&
			    count == PAGE_SIZE) {
			   ...
			   int zeroing)
{
	...
	for (i = 0; i < ap->num_pages && (nbytes || zeroing); i++) {
		...
		last_bytes = (i != ap->num_pages - 1) ? last_bytes : nbytes;
...
static int fuse_copy_args(struct fuse_copy_state *cs, unsigned numargs,
			  unsigned argpages, struct fuse_arg *args, int zeroing)
{
	int err = 0;
	unsigned i;
...
restart:
	for (;;) {
		...
		if (file->f_flags & O_NONBLOCK)
			return -EAGAIN;
		err = wait_event_interruptible_exclusive(
			fiq->waitq, !fiq->connected || request_pending(fiq));
		if (err)
			return err;
	}
				    ...
				    unsigned int flags)
{
	...
	if (pipe_occupancy(pipe->head, pipe->tail) + cs.nr_segs >
	    pipe->max_usage) {
		ret = -EIO;
		goto out;
	}
...
{
	int err = -ENOMEM;
	fuse_req_cachep = kmem_cache_create(
		"fuse_request", sizeof(struct fuse_req), 0, 0, NULL);
	if (!fuse_req_cachep)
		goto out;
