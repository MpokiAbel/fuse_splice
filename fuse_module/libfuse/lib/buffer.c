							   ...
							   size_t len, enum fuse_buf_copy_flags flags)
{
	...
	// Newly defined
	// if (src->flags & FUSE_BUF_FD_SECTION)
	// {
	// 	section.flags = splice_flags;
	// 	section.len = len;
	// 	section.metadata_len = src->metadata;
	// 	section.payload_len = len;
	// 	section.footer_len = src->footer;
	// 	buf = (char *)malloc((section.footer_len + section.metadata_len) * sizeof(char));
	// 	printf("In section printing %ld\n", len);
	// }
	...
	while (len)
	{
		// if (src->flags & FUSE_BUF_FD_SECTION)
		// {
		// 	res = syscall(452, src->fd, srcpos, dst->fd, dstpos, &section, buf);
		// 	// if (res > 0)
		// 	// 	res = res + section.metadata_len + section.footer_len;
		// 	printf("In section printing again\n");
		// }
	...
	printf("In buf splice copied %ld\n", copied);
	return copied;
}
#else
static ssize_t fuse_buf_splice(const struct fuse_buf *dst, size_t dst_off,
