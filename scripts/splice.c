...
SYSCALL_DEFINE9(spliceM, int, fd_in, loff_t __user *, off_in, int, fd_out,
		 loff_t __user *, off_out, size_t, len, unsigned int, flags,
		 size_t, header_len, size_t, payload_len, size_t, footer_len)
{
	struct fd in, out;
	long error;
	...
	if (in.file) {
		out = fdget(fd_out);
		if (out.file) {
			//Put my logic here before the actual splicing being done.
			//ToDo: extract each payload and splice it

			while (len ) {
				

				error = __do_splice(in.file, off_in, out.file,
						    off_out, payload_len,
						    flags);
			}
