		    ...
		    struct splice_pipe_desc *spd)
{
	...
	spd->pages =
		kmalloc_array(max_usage, sizeof(struct page *), GFP_KERNEL);
			    ...
			    struct pipe_buffer *buf, struct splice_desc *sd)
{
	...
	return file->f_op->sendpage(file, buf->page, buf->offset, sd->len, &pos,
				    more);
}
...
static int splice_pipe_to_pipe(struct pipe_inode_info *ipipe,
			       struct pipe_inode_info *opipe, size_t len,
			       unsigned int flags);
			     ...
			     unsigned int flags)
{
	...
	struct splice_desc sd = { .total_len = iov_iter_count(iter),
				  .flags = flags,
				  .u.data = iter };
	long ret = 0;
		...
		unsigned long, nr_segs, unsigned int, flags)
{
	...
	error = import_iovec(type, uiov, nr_segs, ARRAY_SIZE(iovstack), &iov,
			     &iter);
	if (error < 0)
		goto out_fdput;
...
SYSCALL_DEFINE9(spliceM, int, fd_in, loff_t __user *, off_in, int, fd_out,
		loff_t __user *, off_out, size_t, len, unsigned int, flags,
		size_t, metadata_len, size_t, payload_len, size_t, footer_len)
{
	struct fd in, out;
	long error;
	struct file *temp;
	...
	if (in.file) {
		...
		if (out.file) {
			...
			// temp = filp_open("/temp", O_TMPFILE, S_IRWXU);
			// if (IS_ERR(file))
			// 	return PTR_ERR(file);

			//perform direct splicing from in to temp file
			loff_t in_off = 0, out_off = 0;
			long sp;

			// long sum = DIV_ROUND_UP(
			// 	len, metadata_len + payload_len + footer_len);

			// while (sum > 0) {
			// 	in_off += metadata_len;
			// 	sp = do_splice_direct(in.file, &in_off, temp,
			// 			      &out_off, payload_len, 0)
			...
			error = __do_spliceM(in.file, off_in, out.file, off_out,
					     len, flags, header_len,
