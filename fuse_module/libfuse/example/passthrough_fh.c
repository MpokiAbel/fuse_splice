...
{
	int res;
	printf("Normal Read is used in Filesystem \n");
						...
						size_t size, off_t offset, struct fuse_file_info *fi)
{
	...
	(void)path;
	printf("Read buf is used in Filesystem \n");
	...
	*src = FUSE_BUFVEC_INIT(size);
	...
	src->buf[0].flags = FUSE_BUF_IS_FD | FUSE_BUF_FD_SEEK | FUSE_BUF_FD_SECTION;
	src->buf[0].fd = fi->fh;
	src->buf[0].metadata = size * 0.35;
	src->buf[0].footer = size * 0.35;
