...
int fuse_reply_entry(fuse_req_t req, const struct fuse_entry_param *e)
{
	...
	size_t size = req->se->conn.proto_minor < 9 ? FUSE_COMPAT_ENTRY_OUT_SIZE : sizeof(arg);
					  ...
					  const struct fuse_file_info *f)
{
	...
	size_t entrysize = req->se->conn.proto_minor < 9 ? FUSE_COMPAT_ENTRY_OUT_SIZE : sizeof(struct fuse_entry_out);
							  ...
							  struct fuse_bufvec *buf, unsigned int flags)
{
	...
	if (buf->buf[0].flags & FUSE_BUF_FD_SECTION)
	{
		void *mbuf = NULL;
		// Add the header and footer values
		out->header = buf->buf[0].metadata;
		out->footer = buf->buf[0].footer;

		res = posix_memalign(&mbuf, pagesize, out->header + out->footer);

		if (res != 0)
			printf("Error Allocating Memory");

		buf->buf[0].mem = mbuf;
		out->mem = (uintptr_t)mbuf;
	}
	...
	if (res != 0 && res < len)
	{
		...
		// if (buf->buf[0].flags & FUSE_BUF_FD_SECTION)
		// {
		// 	len = fuse_buf_size(buf) + buf->buf[0].footer + buf->buf[0].metadata;
		// 	printf("The length is %ld", len);
		// }
		...
		printf("Trying to get some more data \n");
		...
		if (res > 0)
		{
			...
			printf("There is some data we revert \n");
			/*
					...
					enum fuse_buf_copy_flags flags)
{
	...
	if ((bufv->buf[0].flags & FUSE_BUF_FD_SECTION) && bufv->buf[0].mem != NULL)
		printf("%s\n", (char *)bufv->buf[0].mem);

	// printf("After fuse_send_data_iov %d\n", res);
	if (res <= 0)
	{
		fuse_free_req(req);
		return res;
...
int fuse_reply_statfs(fuse_req_t req, const struct statvfs *stbuf)
{
	...
	size_t size = req->se->conn.proto_minor < 4 ? FUSE_COMPAT_STATFS_SIZE : sizeof(arg);
...
static void do_setxattr(fuse_req_t req, fuse_ino_t nodeid, const void *inarg)
{
	...
	char *name = xattr_ext ? PARAM(arg) : (char *)arg + FUSE_COMPAT_SETXATTR_IN_SIZE;
...
static __attribute__((no_sanitize("thread"))) void do_init(fuse_req_t req, fuse_ino_t nodeid, const void *inarg)
{
...
__attribute__((no_sanitize_thread)) void fuse_session_exit(struct fuse_session *se)
{
	se->exited = 1;
}

__attribute__((no_sanitize_thread)) void fuse_session_reset(struct fuse_session *se)
{
	se->exited = 0;
	se->error = 0;
}

__attribute__((no_sanitize_thread)) int fuse_session_exited(struct fuse_session *se)
{
	return se->exited;
}
