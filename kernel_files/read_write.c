...
static long __do_spliceM(struct file *in, loff_t __user *off_in,
			 struct file *out, loff_t __user *off_out, size_t len,
			 unsigned int flags)
{
	...
	printk(KERN_INFO "Before Inner Splice : len %ld, \n", len);

	ret = do_splice(in, __off_in, out, __off_out, len, flags);
	printk(KERN_INFO "Inner Splice : %ld \n", ret);
...
SYSCALL_DEFINE6(spliceM, int, fd_in, loff_t __user *, off_in, int, fd_out,
		loff_t __user *, off_out, struct spliceM __user *, data,
		char __user *, buf)
{
	struct fd in, out;
	long error, read_error;
	struct spliceM mydata;

	if (copy_from_user(&mydata, data, sizeof(struct spliceM)))
		return -EFAULT;

	if (unlikely(!mydata.len))
		return 0;

	if (unlikely(mydata.flags & ~SPLICE_F_ALL))
		return -EINVAL;

	error = -EBADF;
	printk(KERN_INFO "Metadata %ld, Payload %ld, Footer %ld\n",
	       mydata.metadata_len, mydata.payload_len, mydata.footer_len);
	in = fdget(fd_in);
	if (in.file) {
		out = fdget(fd_out);
		if (out.file) {
			printk(KERN_INFO "Metadata len : %ld \n",
			       mydata.metadata_len);
			read_error = ksys_read(fd_in, buf, mydata.metadata_len);
			printk(KERN_INFO "data read: %ld\n", error);
			if (read_error >= 0) {
				error = __do_spliceM(in.file, off_in, out.file,
						     off_out,
						     mydata.payload_len,
						     mydata.flags);
				printk(KERN_INFO "spliced data : %ld\n", error);
			}

			if (error >= 0) {
				read_error = ksys_read(
					fd_in, buf + mydata.metadata_len,
					mydata.footer_len);
				printk(KERN_INFO "Payload read: %ld\n", error);
			}
	...
	return read_error < 0 ? read_error : error;
}