...
{
	fuse_dentry_settime(entry, time_to_jiffies(o->entry_valid,
						   o->entry_valid_nsec));
}
		...
		if (!ret) {
			...
			if (outarg.nodeid != get_node_id(inode) ||
			    (bool)IS_AUTOMOUNT(inode) !=
				    (bool)(outarg.attr.flags &
					   FUSE_ATTR_SUBMOUNT)) {
				fuse_queue_forget(fm->fc, forget, outarg.nodeid,
						  1);
				goto invalid;
			}
			spin_lock(&fi->lock);
				...
				void **security_ctx, u32 *security_ctxlen)
{
	...
	err = security_dentry_init_security(entry, mode, &entry->d_name, &name,
					    &ctx, &ctxlen);
	if (err) {
		if (err != -EOPNOTSUPP)
			goto out_err;
			    ...
			    struct file *file, unsigned int flags, umode_t mode)
{
	...
	if (fm->fc->handle_killpriv_v2 && trunc && !(flags & O_EXCL) &&
	    !capable(CAP_FSETID)) {
...
int fuse_allow_current_process(struct fuse_conn *fc)
{
	...
	if (uid_eq(cred->euid, fc->user_id) &&
	    uid_eq(cred->suid, fc->user_id) && uid_eq(cred->uid, fc->user_id) &&
	    gid_eq(cred->egid, fc->group_id) &&
	    gid_eq(cred->sgid, fc->group_id) && gid_eq(cred->gid, fc->group_id))
		return 1;
			   ...
			   struct inode *inode, int mask)
{
	...
	if (fc->default_permissions) {
		...
		if (err == -EACCES && !refreshed) {
			err = fuse_perm_getattr(inode, mask);
			if (!err)
				err = generic_permission(&init_user_ns, inode,
							 mask);
		}
		    ...
		    struct file *file)
{
	...
	if (is_wb && attr->ia_valid & (ATTR_MODE | ATTR_UID | ATTR_GID |
			...
			struct iattr *attr)
{
	...
	if (attr->ia_valid & (ATTR_KILL_SUID | ATTR_KILL_SGID)) {
		attr->ia_valid &=
			~(ATTR_KILL_SUID | ATTR_KILL_SGID | ATTR_MODE);
